...
import { VideoSubtitleService } from './subtitle.js';
...
describe('VideoSubtitleService', () => {
  ...
  beforeEach(() => {
    service = new VideoSubtitleService();
    vi.clearAllMocks();
  });
  ...
  describe('isAvailable', () => {
    it('should return true when yt-dlp is available', async () => {
      ...
      expect(mockExecSync).toHaveBeenCalledWith('yt-dlp --version', { stdio: 'pipe' });
    });
  ...
  describe('getAvailableSubtitles', () => {
    it('should return no subtitles message when video has no subtitles', async () => {
      ...
      const result = await service.getAvailableSubtitles('https://youtube.com/watch?v=test');
...
fr       French                  vtt, srt`;
    ...
    it('should handle execution errors', async () => {
      ...
      await expect(service.getAvailableSubtitles('https://youtube.com/watch?v=test'))
